...
var DEFAULT_UPLOAD_SIZE_LIMIT_BYTES = 1024 * 1024 * 1024;
...
function addFileToFormData(uri, name, contentType) {
  ...
  if (isReadableStream2(uri) || isBuffer2(uri) || isFileWebAPI(uri) || isBlobWebAPI(uri)) {
...
function CheckSignature(body, secret, signature) {
  ...
  const hash = import_crypto2.default.createHmac("sha256", key).update(body).digest("hex");
  try {
    return import_crypto2.default.timingSafeEqual(Buffer.from(hash), Buffer.from(signature));
  } catch {
    return false;
  }
}
...
var NotificationManager = class {
  ...
  add({ message, origin: origin2, options = {} }) {
  ...
  addError({ message, origin: origin2, options }) {
    return this.add({ message, origin: origin2, options: { ...options, severity: "error" } });
  }
  addWarning({ message, origin: origin2, options }) {
    return this.add({ message, origin: origin2, options: { ...options, severity: "warning" } });
  }
  addInfo({ message, origin: origin2, options }) {
    return this.add({ message, origin: origin2, options: { ...options, severity: "info" } });
  }
  addSuccess({ message, origin: origin2, options }) {
    return this.add({ message, origin: origin2, options: { ...options, severity: "success" } });
  }
  remove(id) {
